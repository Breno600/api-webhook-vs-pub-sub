...
# - Copiar package.yml para scripts/package/
# - Baixar components do Nexus para /opt/SoftwareExpress/sitef-pipeline/deploy/<path>
# - Copiar scripts do pacote para .../deploy/scripts
# - Executar init_parallel.sh na máquina alvo com as env_vars
# - Gravar status.json com status + log_path (pensando em Harness File Store)
# =====================================================================================
...
- name: "Registrar host dinâmico para {{ current_machine }}"
  ansible.builtin.add_host:
    ...
    ansible_host: "{{ machine_cfg.host | default(machine_cfg.ip | default(machine_cfg.ansible_host | default(''))) }}"
    ansible_user: "{{ target_user }}"
    ansible_ssh_common_args: "{{ ssh_common_args }}"
  changed_when: true

# -----------------------------------------------------------------------------
# 15) Definir caminhos de status e deploy
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 16) Calcular caminhos para File Store (Harness)
# -----------------------------------------------------------------------------
- name: "Calcular caminhos de status/log no File Store"
  ansible.builtin.set_fact:
    filestore_log_path: >-
...
- name: "Copiar package.yml para o host"
  ansible.builtin.copy:
    ...
    dest: "{{ deploy_scripts_package_dir }}/package.yml"
    mode: "0644"
  become: true
  delegate_to: "{{ current_machine }}"

# -----------------------------------------------------------------------------
# 21) Baixar componentes do Nexus para /deploy/<path do componente>
# -----------------------------------------------------------------------------
...
#     Ex.: scripts/deploy-sitef-0.0.2 -> /deploy/scripts/
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 26) (Opcional) Ponto para integrar upload no Harness File Store
#      Aqui você pode chamar um script/cli que faz o upload do JSON e LOG
# -----------------------------------------------------------------------------
# - name: "Upload de status/log para Harness File Store"
#   ansible.builtin.command: >
#     /usr/local/bin/harness_filestore_uploader
#     --status "{{ machine_status_file }}"
#     --log    "{{ deploy_scripts_dir }}/parallel.txt"
#     --status-path "{{ filestore_status_path }}"
