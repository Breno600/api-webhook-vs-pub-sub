...
for gid in "${all_groups[@]}"; do
  ...
  for ((i=0;i<pcount;i++)); do
    ...
    # ===== pipelines (NORMALIZAÇÃO CRÍTICA) =====
    pls_raw="$(api_get "/projects/$pid/pipelines" "per_page=1&order_by=updated_at&sort=desc" || true)"
    if ! echo "${pls_raw:-}" | jq -e 'type=="array"' >/dev/null 2>&1; then
      log "WARN: pipelines retornou não-array para $ppath (talvez 401/403/429/500); tratando como []."
    fi
    pls="$(echo "${pls_raw:-[]}" | jq -c 'if type=="array" then . else [] end' 2>/dev/null || echo '[]')"
...
final_json="$(
  jq -n \
    ...
    '{ generated_at:$generated_at, gitlab_url:$gitlab_url,
